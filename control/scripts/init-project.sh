...
ROOT="$SCRIPT_ROOT"
if [[ "$(basename "$SCRIPT_ROOT")" == "control" && "$(basename "$(dirname "$SCRIPT_ROOT")")" == ".ai-control" ]]; then
...
usage() {
  cat <<'USAGE'
Usage:
  bash .ai-control/control/scripts/init-project.sh [--name <project-name>] [--force] [project-root]
...
if [[ -x "$SCRIPT_ROOT/scripts/detect-project-profile.sh" ]]; then
  "$SCRIPT_ROOT/scripts/detect-project-profile.sh" --write --name "$PROJECT_NAME" "$ROOT" >/dev/null
  echo "[WRITE] .ai-control/project.env"
  echo "[WRITE] .ai-control/project-profile.md"
fi
...
- 模式：待确认。
- 本服务是否建设 RBAC：待确认。
- 说明：可以在 \`.ai-control/project.env\` 中把 \`ACCESS_CONTROL_MODE\` 设置为 \`local\`、\`external\` 或 \`none\`；后台功能实现前必须确认访问控制方案。
...
## 待确认问题
- 用户角色和权限边界是什么。
- 本服务是否建设 RBAC，或由外部系统、网关、IAM/SSO 负责访问控制。
- 是否存在租户、组织或数据隔离规则。
...
## 已确认技术上下文
- Detected: $TECH_STACK
- 统一验证入口：\`bash .ai-control/control/scripts/run-tests.sh\`。

## 必须确认后才能实现的规则
- 角色和权限边界。
- 访问控制模式：本服务 RBAC、外部系统负责或明确不适用。
- 租户和数据隔离规则。
- 状态流转。
- 删除、撤回、作废和归档规则。
