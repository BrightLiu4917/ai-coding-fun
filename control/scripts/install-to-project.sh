...
usage() {
  cat <<'USAGE'
Usage:
  bash control/scripts/install-to-project.sh [options] /path/to/project
...
target_item_path() {
  local item="$1"
  case "$item" in
    AGENTS.md|CONTEXT.md|CONTEXT-MAP.md|openspec|openspec/*)
      printf '%s\n' "$item"
      ;;
    *)
      printf '.ai-control/control/%s\n' "$item"
      ;;
  esac
}
...
copy_path() {
  ...
  local src="$ROOT/$item"
  local target_item
  target_item="$(target_item_path "$item")"
  local dst="$TARGET/$target_item"
  ...
  if [[ -e "$dst" ]]; then
    ...
    if [[ "$BACKUP" -eq 1 ]]; then
      log "[BACKUP] $target_item -> .agent/install-backup/$RUN_ID/$target_item"
      if [[ "$DRY_RUN" -eq 0 ]]; then
        mkdir -p "$(dirname "$backup_root/$target_item")"
        cp -R "$dst" "$backup_root/$target_item"
      fi
      backed_up=$((backed_up + 1))
    elif [[ "$FORCE" -ne 1 ]]; then
      log "[CONFLICT] $target_item"
      conflicts=$((conflicts + 1))
      return
    fi

    if [[ "$FORCE" -eq 1 || "$BACKUP" -eq 1 ]]; then
      log "[REPLACE] $target_item"
      if [[ "$DRY_RUN" -eq 0 ]]; then
        rm -rf "$dst"
      fi
    fi
  else
    log "[INSTALL] $item -> $target_item"
  fi

  if [[ "$DRY_RUN" -eq 0 ]]; then
    mkdir -p "$(dirname "$dst")"
    if [[ "$item" == "openspec" ]]; then
      mkdir -p "$dst/changes" "$dst/specs"
      cp "$src/config.yaml" "$dst/config.yaml"
      cp "$src/project.md" "$dst/project.md"
      [[ -f "$src/changes/README.md" ]] && cp "$src/changes/README.md" "$dst/changes/README.md"
      if [[ -d "$src/specs" ]]; then
        cp -R "$src/specs/." "$dst/specs/"
      fi
    else
      cp -R "$src" "$dst"
    fi
