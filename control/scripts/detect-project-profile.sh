...
usage() {
  cat <<'USAGE'
Usage:
  bash .ai-control/control/scripts/detect-project-profile.sh [--write] [--name <project-name>] [project-root]
...
- \`ACCESS_CONTROL_MODE\` 默认是 \`pending\`；运行 \`bash .ai-control/control/scripts/ai-dev.sh init\` 后可改为 \`local\`、\`external\` 或 \`none\`。
EOF
