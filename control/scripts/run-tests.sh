...
AI_CONTROL_ENV_FILE="$PROJECT_ROOT/.ai-control/project.env"
PROJECT_ENV_FILE="$PROJECT_ROOT/.agent/project.env"
...
is_self_test_command() {
  ...
  printf '%s\n' "$command" | grep -Eq '(^|[[:space:];|&])((bash|sh)[[:space:]]+)?(\./)?(.ai-control/control/)?scripts/run-tests\.sh([[:space:];|&]|$)'
}
