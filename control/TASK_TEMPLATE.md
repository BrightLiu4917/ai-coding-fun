...
## Matt-style 工程方法
- 需求不清：用 `agent-spec` 做追问、术语校准和场景压力测试。
- 需要进入规格流程：用 `agent-spec` 创建或完善 OpenSpec change。
- 需要把追问结果落到 OpenSpec：用 `agent-spec`。
...
- 详细 agent 路由、并行、循环、回退和冲突处理：按 `.ai-control/control/docs/AGENT_ROUTING.md`。
