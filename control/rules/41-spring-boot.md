...
本文件是 Spring Boot 技术栈入口规则；详细实现约束以 `.ai-control/control/rules/41-spring-boot.md` 为准。

读取顺序：

1. 先读 `.ai-control/control/rules/41-spring-boot.md` 判断是否适用。
2. 再读 `.ai-control/control/rules/41-spring-boot.md` 获取依赖分级、ID、异常、事务和 DTO/VO 细则。
